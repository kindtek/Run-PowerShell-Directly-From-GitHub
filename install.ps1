...
$null = New-Item -Path "$devels_advocate/scripts" -ItemType Directory -Force -ErrorAction SilentlyContinue 
...
Write-Host "`n`rDownloading: $github_domain/$dir_host_devels_advocate/scripts/$add_windows_features`r`nDestination: $local_devels_advocate/scripts/$add_windows_features`n`r" -ForegroundColor Magenta 
$WebClient.DownloadFile("$github_domain/$dir_host_devels_advocate/scripts/$add_windows_features", "$local_devels_advocate/scripts/$add_windows_features")
