    ...
    do {
        ...
        if ($confirmation -eq '') {  
            ...
            if (!(Test-Path -Path "$git_path/.dvlp-installed" -PathType Leaf)) {
                ...
                Start-Process powershell -ArgumentList "-command &{. $git_path/powerhell/download-everything-and-install.ps1;install_winget $git_parent_path; install_repo $git_parent_path $git_path $repo_src_ownr $repo_src_name $repo_dir_name $repo_src_branch;. $git_path/scripts/install-everything.ps1;run_installer;exit;}" 
            }
